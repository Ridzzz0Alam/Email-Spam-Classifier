...
function render(text, res) {
  ...
  els.coverage.textContent = res.words_total === 0
    ...
    : `The model recognised ${fmtInt(res.words_recognised)} of the ${fmtInt(res.words_total)} ` +
      ...
      `stay unshaded. Shading is relative to the strongest word in this email; hover a word ` +
      `for its exact contribution.`;
...
els.annotated.addEventListener("mouseout", (e) => {
  if (e.target.closest(".w")) els.tip.hidden = true;
});
els.annotated.addEventListener("scroll", () => { els.tip.hidden = true; });
